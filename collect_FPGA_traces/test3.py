...
ser = serial.Serial('COM47',115200,xonxoff=0, rtscts=0,bytesize=8, parity='N', stopbits=1)
...
for m in range(0,numberOfTraces):
	# if m==0:
		# key=0x2322A4DC760D8C181E2E99B4E0BF12A294AC7F7A3E0136876C57A680CA9F7F58
		# cons=0x617078653320646e79622d326b206574
		# nonce = int(T_nounces[m,0]) << 96 | int(T_nounces[m,1]) << 64 | int(T_nounces[m,2]) << 32 | int(T_nounces[m,3])
	# else:	
		# nonce = int(T_nounces[m,0]) << 96 | int(T_nounces[m,1]) << 64 | int(T_nounces[m,2]) << 32 | int(T_nounces[m,3])	
	#data_in = (cons<< 384) | (key<<128) | (ctr<<96) | nonce
	#just for testing one trace
	#data_in = 0xffffffff0000000179622d326b206574030201000706050407345432 
	data_in = int(random.randint(0,0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF))
	#print hex(data_in  >> 192)
	#print hex(data_in >> 160 & 0xFFFFFFFF )
	x= (data_in  >> 192 & 0xFFFFFFFF) ^ (data_in  >> 128 & 0xFFFFFFFF) ^ (data_in  >> 96 & 0xFFFFFFFF)
	y= (data_in  >> 160 & 0xFFFFFFFF) ^ (data_in  >> 64 & 0xFFFFFFFF) ^ (data_in  >> 32 & 0xFFFFFFFF)
	print "x:" + hex(x)
	print "y:" + hex(y)
	results = x + y
